# 问题在于：原文中使用的LED灯的位置是间隔1cm分布的，而这个仿真默认的是所有灯都位于中心位置，从而导致误差。
######################################################################################
# 2018.11.28： 这个程序可以用来计算确定的LED位置下，房间中的亮度情况（例如Max、Min、Avg等）
# 2018.12.1： 这个部分运行速度可以不作要求，因为实际上这个产生完数据之后就结束了，不会继续用到这个部分
#
#
#
